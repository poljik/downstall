...
begin {
  ...
  function Import-EnvironmentVariables {
    ...
    if (Test-Path $EnvPath) {
      foreach ($line in (Get-Content -Path $EnvPath -Encoding UTF8)) {
        # Skip empty lines for better readability
        ...
        if ($null -eq $Value) {
          ...
          $Value = $Value.Trim()
          # Remove surrounding matching quotes only if both are present (single or double)
  ...
  $Script:SystemArchitecture = if ([System.Environment]::Is64BitOperatingSystem) {
    64
  }
  else {
    86
      ...
      try {
        ...
        while (($BytesRead = $Stream.Read($Buffer, 0, $Buffer.Length)) -gt 0) {
          ...
          if ($SwUI.ElapsedMilliseconds -ge 500) {
            ...
            $Percent = if ($TotalSize -gt 0) {
              [int](($TotalRead / $TotalSize) * 100)
            }
            else {
              0
            }
            $ReadMB = "{0:N1}" -f ($TotalRead / 1MB)
            $TotalMB = if ($TotalSize -gt 0) {
              "{0:N1}" -f ($TotalSize / 1MB)
            }
            else {
              "?"
            }
            $RemainingSeconds = if ($SpeedBps -gt 0 -and $TotalSize -gt 0) {
              [int](($TotalSize - $TotalRead) / $SpeedBps)
            }
            else {
              -1
            }
  ...
  function Test-FileUpdateRequired {
    ...
    $SkipCheck = if ($PSVersionTable.PSVersion.Major -ge 6) {
    ...
    $NewSizeLong = if ($NewSize -is [Array]) {
      [long]$NewSize[0]
    }
    else {
      [long]$NewSize
  ...
  function Invoke-Office365Setup {
    ...
    if (-not ([System.Environment]::OSVersion.Platform -eq 'Win32NT')) {
      Write-Warning "Office365Setup is designed for Windows operating systems only. You are running $([System.Environment]::OSVersion.Platform). Skipping Office365 setup."
      return
    }
...
"@
  ...
  function Invoke-PostInstall {
    ...
    switch -Wildcard ($SoftwareName) {
      ...
      "office*" {
        @("Word.lnk", "Excel.lnk", "Powerpoint.lnk") | ForEach-Object {
          ...
          if (Test-Path $ShortcutPath) {
            Copy-Item -Path $ShortcutPath -Destination $CommonDesktop -Force
          }
        }
      }
      "true_image" {
      ...
      "viber" {
        $Retry = 0
        while ((Get-Process | Where-Object Path -match "vibersetup" -ErrorAction SilentlyContinue) -and $Retry -lt 60) {
          Start-Sleep -Seconds 1; $Retry++
        }
      ...
      "platelschik_eaes_rate" {
        ...
        $SetupDir = $Null
        if (Test-Path $EaesPath) {
          $SetupDir = (Get-Item -Path $EaesPath -Force | Select-Object -First 1).FullName
        }
        else {
        ...
        if ($SetupDir) {
          ...
          Write-Warning "Can't find platelschik_eaes directory for rate update."
        }
      }
      "rdpwrap_ini" {
  ...
  function Install-SoftwarePackage {
    ...
    foreach ($Pattern in $CustomInstallOnly) {
      if ($SoftwareName -like $Pattern) {
        $BypassStandardInstall = $true; break
      }
    }
      ...
      if ($IsArchive) {
        ...
        switch -Regex ($Extension) {
          ...
          "\.(rar|7z)$" {
            ...
            if (Test-Path $WinRarPath) {
              $WinRarArgs = if ($SoftwareItem.ArchiveArgs) {
                "x -o+ $($SoftwareItem.ArchiveArgs)"
              }
              else {
                "x -o+"
              }
        ...
        if ($ExtractSuccess) {
          if ($SoftwareItem.SetupPattern) {
            ...
            if ($ExtractedItems) {
              ...
              if ($ExtractSingleExe) {
                ...
                $LaunchArgs = if ($SoftwareItem.InstallArgs) {
                  $SoftwareItem.InstallArgs
                }
                else {
                  ""
                }
                ...
                if ($SoftwareName -eq "avest") {
                  ...
                  if ($AvestSetup) {
                    Start-Process -FilePath $AvestSetup.FullName -ArgumentList "/verysilent /devices=avToken,avPass,iKey" -Wait -ErrorAction SilentlyContinue
                  }
                }
              }
            }
          ...
          elseif ($ExtractSingleExe) {
            ...
            if ($FirstExe) {
              Copy-Item -Path $FirstExe.FullName -Destination $CommonDesktop -Force -ErrorAction SilentlyContinue
            }
          }
  ...
  function Update-DynamicSoftwareUrls {
    ...
    foreach ($Soft in $SoftwareArray) {
      if ($Soft.SoftwareName -notin $ActiveInstalls) {
        continue
      }
      switch ($Soft.SoftwareName) {
        { $_ -match "^irfanview" } {
          if (-not $IrfanViewVersion) {
            ...
            if ($Match.Success) {
              $IrfanViewVersion = $Match.Groups[1].Value.Replace(".", "")
            }
          }
    ...
    if ($DownloadUrl -and -not $InstallOnly) {
      ...
      elseif ($DownloadUrl -match "drive\.google\.com/file/d") {
        ...
        if ($TitleMatch.Success) {
          $LinkFileName = $TitleMatch.Groups[1].Value
        }
      ...
      elseif ($DownloadUrl -match "^https?://github\.com/([^/]+)/([^/]+)") {
        ...
        if (-not ([string]$AbsUrl = Get-AbsoluteUri -DownloadUrl $DownloadUrl)) {
          return
        }
        $DownloadUrl = $AbsUrl
        $DownloadLink = $DownloadUrl
        ...
        $Pattern = if ($SoftwareItem.SoftwareName -eq "imagine") {
          "*download.php?arch=x64&unicode=1&full=0&setup=1*"
        }
        else {
          $SoftwareItem.SearchPattern
        }
        $FoundLinks = @($WebResponse.links.href | Where-Object { $_ -like $Pattern })

        if ($FoundLinks.Count -gt 0) {
          $DownloadLink = [string]$FoundLinks[0]
          if ($SoftwareItem.SoftwareName -eq "vvoddpu" -and $FoundLinks[0] -like "*0.zip") {
            $DownloadLink = [string]$FoundLinks[1]
          }
          if ($SoftwareItem.SoftwareName -eq "victoria") {
            $DownloadLink = [string]$FoundLinks[1].Substring($FoundLinks[1].LastIndexOf("https:"))
          }

          if (-not $DownloadLink.StartsWith("http")) {
            if ($DownloadLink.Contains("/")) {
              $DownloadLink = if ($DownloadLink.StartsWith("/")) {
                $DownloadUrl.Substring(0, $DownloadUrl.IndexOf("/", 8)) + $DownloadLink
              }
              else {
                $DownloadUrl.Substring(0, $DownloadUrl.IndexOf("/", 8) + 1) + $DownloadLink
              }
            }
            else {
